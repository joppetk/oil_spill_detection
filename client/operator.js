...
const MAX_KM2 = 500; // <-- change your cap here for the AOI area
...
// actions
/* document.getElementById('btn-set-home').onclick = async () => {
  const clientId = elDroneId.value;
  const lat = parseFloat(elHomeLat.value), lon = parseFloat(elHomeLon.value);
  if (Number.isFinite(lat) && Number.isFinite(lon)) {
    // this sets "software home" on the Pi service (you'll add this endpoint/command on the Pi)
    try {
      await sendCtrlAndWait(clientId, { cmd: 'set_home_soft', lat, lon });
      alert('Home set (software). Note: PX4 FCU home still set on arm.');
...
const IncidentsUI = (() => {
  // DOM refs (lazy)
  let tableBody, filterSel, refreshBtn, detailBox, titleEl, stateEl, priorityEl, verifEl, areaEl,protEl,portEl,desalEl, distEl, tierLevelEl,incidentDescriptionEl,tlEl, taskBtn, confirmBtn, refuteBtn, unsureBtn, planBtn, responseBtn,rulesBody;
